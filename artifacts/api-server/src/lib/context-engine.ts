...
export async function buildContextPacket(query: string, mode: ConversationMode, budgetTokens = 3000, intent?: { retrievalMode?: string }) {
  ...
  const items = [
    ...memoryItems.map((item) => {
      ...
      return { id: record.id, text, kind: record.objectType ?? "semantic", confidence: record.propagatedConfidence ?? record.confidence ?? record.similarityScore ?? 0.5, recencyDays: Math.max(0, (Date.now() - new Date(updatedAt).getTime()) / 86400000), strategicAnchor: protectedTier, memoryTier: record.memoryTier };
    }),
    ...facts.map((item) => ({ id: item.id, text: `Fact · ${item.subject} ${item.predicate} ${item.object} [${item.factType}]`, kind: "fact", confidence: item.propagatedConfidence ?? item.confidence, recencyDays: Math.max(0, item.updatedAt ? (Date.now() - new Date(item.updatedAt).getTime()) / 86400000 : 0), strategicAnchor: item.canonLevel === "canonical" })),
    ...interpretations.map((item) => ({ id: item.id, text: `Interpretation · ${item.statement} [${item.interpretationType}]`, kind: "interpretation", confidence: item.propagatedConfidence ?? item.confidence, recencyDays: Math.max(0, item.updatedAt ? (Date.now() - new Date(item.updatedAt).getTime()) / 86400000 : 0), strategicAnchor: item.canonLevel === "canonical" })),
