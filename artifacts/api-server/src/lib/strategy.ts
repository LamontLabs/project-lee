...
export async function runSimulation(question: string, simulationType = "general") {
  ...
  const relatedProjects = projects.filter((project) => normalized.includes(String(project.name ?? project.title ?? "").toLowerCase())).map((project) => ({ projectId: project.id, impact: "Potential sequencing, readiness, or dependency impact requires owner review." }));
  ...
  const assumptions = [{ assumptionId: assumption.id, statement: assumption.statement, confidence: assumption.confidence }, { statement: "This simulation is advisory and does not execute actions.", confidence: 1 }];
