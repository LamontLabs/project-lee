...
export default function WaitingTab() {
  ...
  const { waiting, hosted, waitingAction, refresh } = useLee();
  const loops = waiting?.value ?? [];
  ...
  return (
    <Screen refreshing={false} onRefresh={() => void refresh()}>
      ...
      }) : <Card><Text style={[styles.body, { color: colors.mutedForeground }]}>{hosted.freshness === 'unavailable' ? 'Hosted waiting loops are unavailable. No cached loops are available.' : 'Nothing is currently waiting on a person or system.'}</Text></Card>}
    </Screen>
  );
}
