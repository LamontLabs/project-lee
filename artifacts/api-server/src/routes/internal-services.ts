...
router.post("/internal-services/cil/query", async (req, res) => {
  try { return res.json(await reasoningService.query(req.body)); } catch (error) { return res.status(503).json({ error: "CIL unavailable; route to frontier fallback.", detail: String(error) }); }
});
