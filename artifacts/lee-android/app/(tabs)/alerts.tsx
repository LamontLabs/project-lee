...
export default function AlertsTab() {
  ...
  const { alerts, hosted, authorizedNotificationTarget, alertAction, refresh } = useLee();
  const { id } = useLocalSearchParams<{ id?: string }>();
  const allItems = alerts?.value ?? [];
  const items = id
    ? authorizedNotificationTarget?.destination === 'alerts' && authorizedNotificationTarget.id === id
      ? authorizedNotificationTarget.records.filter((alert) => alert.id === id)
      : []
    : allItems;
  ...
  return (
    <Screen refreshing={false} onRefresh={() => void refresh()}>
      ...
      </Card>) : <Card><Text style={[styles.body, { color: colors.mutedForeground }]}>{id ? 'This alert is no longer available from the live Owner service.' : hosted.freshness === 'unavailable' ? 'Hosted alerts are unavailable. No cached alerts are available.' : 'No alerts are currently above the digest level.'}</Text></Card>}
    </Screen>
  );
}
