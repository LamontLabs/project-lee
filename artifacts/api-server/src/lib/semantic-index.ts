...
import { db, eventLog, factLedger, interpretationLedger, semanticIndex, universalObject } from "@workspace/db";
...
export async function indexObject(objectId: string, objectType: string) {
  ...
  const object = await sourceRecord(objectId, objectType); if (!object) return null; const record = object as any; const text = representation(objectType, record); const sourceUpdatedAt = objectType === "event" ? record.occurredAt : record.updatedAt ?? record.createdAt; const [row] = await db.insert(semanticIndex).values({ objectId, objectType, embedding: vectorize(text), indexedAt: new Date(), sourceUpdatedAt, modelVersion: MODEL_VERSION, excerpt: text.slice(0, 500) }).onConflictDoUpdate({ target: [semanticIndex.objectId, semanticIndex.objectType], set: { embedding: vectorize(text), indexedAt: new Date(), sourceUpdatedAt, modelVersion: MODEL_VERSION, excerpt: text.slice(0, 500) } }).returning(); return row;
}
