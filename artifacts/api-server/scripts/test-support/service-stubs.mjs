...
export async function startInternalServiceStubs() {
  const calls = [];
  const server = http.createServer(async (request, response) => {
    ...
    if (request.url === "/v1/query") return response.end(JSON.stringify({ correlation_id: calls.at(-1).body.correlation_id, resolution_tier: "T1_TRIGRAM", answer: "stub answer", confidence: 0.95, cost_usd: 0, latency_ms: 1, semantic_domain: "test", reuse_eligible: true, drift_detected: false, contradiction_detected: false, provenance: ["stub"], governance_status: "approved", freshness_state: "fresh", recommend_escalation: false }));
