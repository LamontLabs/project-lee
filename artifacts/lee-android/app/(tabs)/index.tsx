  ...
  return (
    <Screen refreshing={refreshing} onRefresh={refresh}>
      ...
      <Card style={{ backgroundColor: colors.primary, borderColor: colors.primary }}>
       ...
       <Card><View style={styles.row}><View style={[styles.iconCircle, { backgroundColor: connections.some((item) => item.status !== 'connected') ? colors.secondary : colors.accent }]}><Feather name="link" size={18} color={connections.some((item) => item.status !== 'connected') ? colors.secondaryForeground : colors.primary} /></View><View style={styles.flex}><Text style={[styles.cardTitle, { color: colors.foreground }]}>Connected systems</Text><Text style={[styles.body, { color: colors.mutedForeground }]}>{connections.length ? `${connections.length} registered · ${connections.filter((item) => item.status === 'connected').length} healthy` : 'No connected systems reported.'}</Text>{connections.filter((item) => item.status !== 'connected').slice(0, 2).map((item) => <Text key={item.id} style={[styles.body, { color: colors.destructive, marginTop: 4 }]}>{item.displayName}: {item.health?.whatFailed ?? item.health?.summary ?? `Status ${item.statusLabel ?? item.status}`}{item.health?.ownerActionRequired ? ' Owner action required.' : item.health?.recoveryAutomatic ? ' Lee will retry automatically.' : ''}</Text>)}</View></View></Card>
