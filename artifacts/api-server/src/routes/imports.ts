...
router.post("/imports", async (req, res): Promise<void> => {
  const { filename, mimeType, content, metadata } = req.body ?? {};
  if (typeof filename !== "string" || typeof mimeType !== "string" || typeof content !== "string") {
    res.status(400).json({ error: "filename, mimeType, and content are required." });
    return;
  }
  try {
    const result = await importSource({ filename, mimeType, content, metadata });
...
router.post("/imports/:sourceId/retry", async (req, res): Promise<void> => {
  ...
  if (!source?.rawContent) { res.status(404).json({ error: "Source content not available for retry." }); return; }
  const result = await importSource({ filename: source.originalFilename, mimeType: source.mimeType, content: source.rawContent, metadata: source.metadata });
  res.json(result);
});
