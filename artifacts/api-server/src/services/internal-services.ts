...
export type GovernedResponse = { verdict: "ALLOW" | "HOLD" | "REJECT"; reason_codes: string[]; checked_invariants: unknown[]; missing_approvals?: unknown[]; remediation_requirements?: string[]; decision_id: string; decision_envelope: string; evidence_bundle_ref: string; audit_entry_ref: string; policy_version: string; timestamp: string; replay_checksum: string; authorization_expiry?: string; human_confirmation_required: boolean };

function signedHeaders(body: string, credential: string | undefined, correlationId: string) {
  const timestamp = Math.floor(Date.now() / 1000).toString(); const digest = createHash("sha256").update(body).digest("hex");
  const signature = createHmac("sha256", credential ?? "").update(`${correlationId}.${timestamp}.${digest}`).digest("hex");
  return { "content-type": "application/json", "X-LEE-Identity": "lee", "X-LEE-Correlation-Id": correlationId, "X-LEE-Timestamp": timestamp, "X-LEE-Signature": signature, ...(credential ? { authorization: `Bearer ${credential}` } : {}) };
}
...
async function requestJson(serviceId: string, url: string, body: unknown, timeoutMs = 8000) {
  ...
  try { const response = await fetch(url, { method: "POST", headers: signedHeaders(encoded, credential, correlationId), body: encoded, signal: controller.signal }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
