...
type Connection = {
  ...
  statusLabel?: string;
  permissions: string[]; capabilities: string[]; credentialConfigured: boolean;
  ...
  freshness?: { state: string; label: string; lastSuccessfulRefreshAt: string | null; evidenceAgeMs: number | null; limitations: string[]; lastReconnectedAt: string | null };
  lastSyncAt?: string | null; lastSuccessfulOperation?: { label: string; at: string } | null; lastError?: string | null;
  diagnostics?: { baseUrl: string | null; healthEndpoint: string | null; contractVersion: string | null; grantedScopes: string[]; dependencies: Array<Record<string, unknown>>; configuration: Record<string, unknown> };
};
...
const displayCapability = (value: string) => value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
...
function ConnectionDetails({ selected, importing, startOAuth, importFiles, mutate, togglePermission }: {
  selected: Connection;
  importing: boolean;
  startOAuth: () => Promise<void>;
  importFiles: (event: React.ChangeEvent<HTMLInputElement>, kind: string) => Promise<void>;
  mutate: (url: string, init?: RequestInit, success?: string) => Promise<void>;
  togglePermission: (permission: string) => void;
}) {
  return <div>
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><p className="lee-label text-primary">{selected.targetType.replaceAll("_", " ")} / {selected.method.replaceAll("_", " ")}</p><h3 className="mt-2 text-2xl font-semibold">{selected.displayName}</h3></div>
      <span className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${tone(selected.status)}`}>{selected.statusLabel ?? statusCopy[selected.status] ?? selected.status}</span>
    </div>
    <div className="mt-4 rounded-xl border border-border bg-muted/30 p-4">
      <p className="text-sm font-semibold">{selected.health?.summary ?? "Connection health has not been assessed yet."}</p>
      <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{selected.health?.remainsAvailable ?? "Connection metadata remains available."}</p>
      {selected.health?.blocked && <p className="mt-2 text-xs leading-relaxed text-destructive">{selected.health.blocked}</p>}
      {selected.health?.ownerActionRequired && <p className="mt-2 text-xs font-semibold text-accent-foreground">Owner action required.</p>}
    </div>
    <div className="mt-3 rounded-xl border border-border bg-background/50 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold">External evidence freshness</p><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${selected.freshness?.label === "current" ? "border-primary/25 bg-primary/10 text-primary" : "border-accent/35 bg-accent/10 text-accent-foreground"}`}>{selected.freshness?.label ?? "unverified"}</span></div>
      <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{selected.freshness?.lastSuccessfulRefreshAt ? `Last successful refresh ${formatDate(selected.freshness.lastSuccessfulRefreshAt)}.` : "No successful provider refresh is recorded."}</p>
      {selected.freshness?.limitations?.map((limitation) => <p key={limitation} className="mt-1 text-[11px] leading-relaxed text-muted-foreground">· {limitation}</p>)}
    </div>
    {selected.method === "oauth" && <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/5 p-4"><p className="text-sm font-semibold">Secure sign-in</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">LEE opens the provider sign-in page. Credentials and granted scopes stay on the server and are never copied to this device.</p><button onClick={() => void startOAuth()} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-3.5 py-2.5 text-xs font-semibold text-primary-foreground">{selected.status === "needs_reauthorization" ? <RefreshCw size={14} /> : <Link2 size={14} />} {selected.status === "needs_reauthorization" ? "Sign in again" : "Start sign-in"}</button></div>}
    {selected.method === "file" && <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/5 p-4"><div className="flex items-start gap-3"><FileUp size={18} className="mt-0.5 text-primary" /><div><p className="text-sm font-semibold">Import approved sources</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Choose files, a folder, repository export, or conversation export. Each item is uploaded privately, then held below canon for review.</p></div></div><div className="mt-4 flex flex-wrap gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-primary px-3.5 py-2.5 text-xs font-semibold text-primary-foreground"><FileUp size={14} /> Files / exports<input type="file" multiple className="hidden" disabled={importing} onChange={(event) => void importFiles(event, "file_or_export")} /></label><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-primary/30 px-3.5 py-2.5 text-xs font-semibold text-primary"><FolderOpen size={14} /> Folder<input type="file" multiple className="hidden" disabled={importing} onChange={(event) => void importFiles(event, "folder")} {...({ webkitdirectory: "", directory: "" } as React.InputHTMLAttributes<HTMLInputElement>)}/></label></div>{importing && <p className="mt-3 text-xs text-primary">Uploading and processing selected sources…</p>}</div>}
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      <div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Authority</p><p className="mt-2 text-sm font-semibold">{selected.authority?.primary ?? "OBSERVE"}</p><p className="mt-1 text-[11px] text-muted-foreground">{selected.authority?.explanation ?? "Connectivity never implies authority."}</p></div>
      <div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Capabilities</p><p className="mt-2 text-sm font-semibold">{selected.capabilities?.length ? selected.capabilities.map(displayCapability).join(" · ") : "None declared"}</p></div>
      <div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Authorization</p><p className="mt-2 text-sm font-semibold">{selected.credentialConfigured ? "Stored securely" : "Not configured"}</p></div>
      <div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Last sync</p><p className="mt-2 text-sm font-semibold">{formatDate(selected.lastSyncAt)}</p></div>
      <div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Last successful operation</p><p className="mt-2 text-sm font-semibold">{selected.lastSuccessfulOperation ? `${selected.lastSuccessfulOperation.label} · ${formatDate(selected.lastSuccessfulOperation.at)}` : "Not recorded"}</p></div>
      <div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Last checked</p><p className="mt-2 text-sm font-semibold">{formatDate(selected.health?.checkedAt)}</p></div>
    </div>
    {selected.health?.whatFailed && <div className="mt-4 flex gap-2 rounded-xl border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive"><CircleAlert size={16} className="mt-0.5 shrink-0" /><div><p className="font-semibold">{selected.health.whatFailed}</p><p className="mt-1 text-xs">Recovery {selected.health.recoveryAutomatic ? "is automatic; LEE will retry." : "requires an owner decision."}</p></div></div>}
    <div className="mt-7 border-t border-border pt-5"><div className="flex items-center justify-between"><div><p className="lee-label text-muted-foreground">Capability permissions</p><p className="mt-1 text-xs text-muted-foreground">Connectivity and authority are separate. OBSERVE always remains enabled.</p></div><span className="text-xs text-primary">{selected.permissions.length} enabled</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{Object.entries(permissionCopy).map(([permission, description]) => <label key={permission} className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-3 hover:bg-muted/50"><input type="checkbox" checked={selected.permissions.includes(permission)} disabled={permission === "OBSERVE"} onChange={() => togglePermission(permission)} className="mt-1 accent-[hsl(var(--primary))]" /><span><span className="block text-xs font-bold">{permission}</span><span className="mt-1 block text-[11px] leading-relaxed text-muted-foreground">{description}</span></span></label>)}</div></div>
    <details className="mt-6 rounded-xl border border-border bg-muted/20 p-4"><summary className="cursor-pointer text-sm font-semibold">Advanced diagnostics <span className="ml-2 text-xs font-normal text-muted-foreground">technical scopes, endpoints, contract, and configuration</span></summary><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><p className="lee-label text-muted-foreground">Endpoint</p><p className="mt-1 break-all font-mono text-[11px] text-muted-foreground">{selected.diagnostics?.baseUrl ?? "Not configured"}{selected.diagnostics?.healthEndpoint ?? ""}</p></div><div><p className="lee-label text-muted-foreground">Contract</p><p className="mt-1 font-mono text-[11px] text-muted-foreground">{selected.diagnostics?.contractVersion ?? "Not reported"}</p></div><div><p className="lee-label text-muted-foreground">Granted scopes</p><p className="mt-1 break-all font-mono text-[11px] text-muted-foreground">{selected.diagnostics?.grantedScopes?.join(", ") || "Not applicable"}</p></div><div><p className="lee-label text-muted-foreground">Dependencies</p><p className="mt-1 text-[11px] text-muted-foreground">{selected.diagnostics?.dependencies?.length ?? 0} declared</p></div><pre className="max-h-48 overflow-auto rounded-lg bg-muted p-3 text-[10px] leading-relaxed text-muted-foreground sm:col-span-2">{JSON.stringify({ configuration: selected.diagnostics?.configuration ?? {} }, null, 2)}</pre></div></details>
    <div className="mt-6 flex flex-wrap gap-2"><button onClick={() => void mutate(`/api/connections/${selected.id}/test`, { method: "POST", body: "{}" }, "Connection test completed.")} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3.5 py-2.5 text-xs font-semibold text-primary-foreground"><Check size={15} /> Test connection</button><button onClick={() => void startOAuth()} className="inline-flex items-center gap-2 rounded-xl border border-border px-3.5 py-2.5 text-xs font-semibold hover:bg-muted">{selected.method === "oauth" ? "Sign in" : "Reauthorize"}</button><button onClick={() => void mutate(`/api/connections/${selected.id}`, { method: "DELETE" }, "Connection disconnected safely.")} className="inline-flex items-center gap-2 rounded-xl border border-destructive/25 bg-transparent px-3.5 py-2.5 text-xs font-semibold text-destructive hover:bg-destructive/5"><Unplug size={15} /> Disconnect</button></div>
  </div>;
}
     ...
     {loading ? <div className="flex items-center gap-2 rounded-2xl border border-border bg-card p-8 text-sm text-muted-foreground"><Loader2 className="animate-spin" size={16} /> Loading connection inventory…</div> : <div className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]">
      <section className="rounded-2xl border border-card-border bg-card/80 p-3 shadow-[0_14px_40px_hsl(205_30%_20%/0.04)]">
        <div className="flex items-center justify-between px-3 pb-3 pt-2"><p className="lee-label text-muted-foreground">{connections.length} connection{connections.length === 1 ? "" : "s"}</p><ShieldCheck size={16} className="text-primary" /></div>
         {connections.length ? <div className="space-y-1">{connections.map((item) => <button key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full rounded-xl border px-3.5 py-3.5 text-left ${selected?.id === item.id ? "border-primary/30 bg-primary/5" : "border-transparent hover:border-border hover:bg-muted/60"}`} data-testid={`button-select-connection-${item.id}`}><div className="flex items-start gap-3"><span className="mt-1 grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-secondary text-secondary-foreground"><Link2 size={15} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{item.displayName}</span><span className="mt-1 block text-xs capitalize text-muted-foreground">{item.targetType.replaceAll("_", " ")} · {item.method.replaceAll("_", " ")}</span><span className={`mt-2 inline-flex rounded-full border px-2 py-1 text-[10px] font-semibold ${tone(item.status)}`}>{item.statusLabel ?? statusCopy[item.status] ?? item.status}</span><span className="mt-2 block truncate text-[11px] text-muted-foreground">{item.authority?.primary ?? "OBSERVE"} authority · {item.capabilities?.length ? item.capabilities.slice(0, 2).map(displayCapability).join(" · ") : "No capabilities declared"}</span></span></div></button>)}</div> : <div className="p-6"><div className="rounded-xl border border-dashed border-border p-6 text-center"><Link2 className="mx-auto mb-3 text-muted-foreground" size={20} /><p className="text-sm font-semibold">No connections yet</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Start with a sign-in or one clear setup flow. LEE will discover the rest.</p></div></div>}
      </section>
         <section className="rounded-2xl border border-card-border bg-card/80 p-6 shadow-[0_14px_40px_hsl(205_30%_20%/0.04)]">{/*
           {selected ? <><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="lee-label text-primary">{selected.targetType.replaceAll("_", " ")} / {selected.method.replaceAll("_", " ")}</p><h3 className="mt-2 text-2xl font-semibold">{selected.displayName}</h3></div><span className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${tone(selected.status)}`}>{selected.statusLabel ?? statusCopy[selected.status] ?? selected.status}</span></div><div className="mt-4 rounded-xl border border-border bg-muted/30 p-4"><p className="text-sm font-semibold">{selected.health?.summary ?? "Connection health has not been assessed yet."}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{selected.health?.remainsAvailable ?? "Connection metadata remains available."}</p>{selected.health?.blocked && <p className="mt-2 text-xs leading-relaxed text-destructive">{selected.health.blocked}</p>}{selected.health?.ownerActionRequired && <p className="mt-2 text-xs font-semibold text-accent-foreground">Owner action required.</p>}</div>{selected.method === "oauth" && <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/5 p-4"><p className="text-sm font-semibold">Secure sign-in</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">LEE opens the provider sign-in page. Credentials and granted scopes stay on the server and are never copied to this device.</p><button onClick={() => void startOAuth()} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-3.5 py-2.5 text-xs font-semibold text-primary-foreground">{selected.status === "needs_reauthorization" ? <RefreshCw size={14} /> : <Link2 size={14} />} {selected.status === "needs_reauthorization" ? "Sign in again" : "Start sign-in"}</button></div>}{selected.method === "file" && <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/5 p-4"><div className="flex items-start gap-3"><FileUp size={18} className="mt-0.5 text-primary" /><div><p className="text-sm font-semibold">Import approved sources</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Choose files, a folder, repository export, or conversation export. Each item is uploaded privately, then held below canon for review.</p></div></div><div className="mt-4 flex flex-wrap gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-primary px-3.5 py-2.5 text-xs font-semibold text-primary-foreground"><FileUp size={14} /> Files / exports<input type="file" multiple className="hidden" disabled={importing} onChange={(event) => void importFiles(event, "file_or_export")} /></label><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-primary/30 px-3.5 py-2.5 text-xs font-semibold text-primary"><FolderOpen size={14} /> Folder<input type="file" multiple className="hidden" disabled={importing} onChange={(event) => void importFiles(event, "folder")} {...({ webkitdirectory: "", directory: "" } as React.InputHTMLAttributes<HTMLInputElement>)}/></label></div>{importing && <p className="mt-3 text-xs text-primary">Uploading and processing selected sources…</p></div>}<div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Authority</p><p className="mt-2 text-sm font-semibold">{selected.authority?.primary ?? "OBSERVE"}</p><p className="mt-1 text-[11px] text-muted-foreground">{selected.authority?.explanation ?? "Connectivity never implies authority."}</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Capabilities</p><p className="mt-2 text-sm font-semibold">{selected.capabilities?.length ? selected.capabilities.map(displayCapability).join(" · ") : "None declared"}</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Authorization</p><p className="mt-2 text-sm font-semibold">{selected.credentialConfigured ? "Stored securely" : "Not configured"}</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Last sync</p><p className="mt-2 text-sm font-semibold">{formatDate(selected.lastSyncAt)}</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Last successful operation</p><p className="mt-2 text-sm font-semibold">{selected.lastSuccessfulOperation ? `${selected.lastSuccessfulOperation.label} · ${formatDate(selected.lastSuccessfulOperation.at)}` : "Not recorded"}</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="lee-label text-muted-foreground">Last checked</p><p className="mt-2 text-sm font-semibold">{formatDate(selected.health?.checkedAt)}</p></div></div>{selected.health?.whatFailed && <div className="mt-4 flex gap-2 rounded-xl border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive"><CircleAlert size={16} className="mt-0.5 shrink-0" /><div><p className="font-semibold">{selected.health.whatFailed}</p><p className="mt-1 text-xs">Recovery {selected.health.recoveryAutomatic ? "is automatic; LEE will retry." : "requires an owner decision."}</p></div></div>}{selected.lastError && !selected.health?.whatFailed && <div className="mt-4 flex gap-2 rounded-xl border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive"><CircleAlert size={16} className="mt-0.5 shrink-0" />{selected.lastError}</div>}<div className="mt-7 border-t border-border pt-5"><div className="flex items-center justify-between"><div><p className="lee-label text-muted-foreground">Capability permissions</p><p className="mt-1 text-xs text-muted-foreground">Connectivity and authority are separate. OBSERVE always remains enabled.</p></div><span className="text-xs text-primary">{selected.permissions.length} enabled</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{Object.entries(permissionCopy).map(([permission, description]) => <label key={permission} className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-3 hover:bg-muted/50"><input type="checkbox" checked={selected.permissions.includes(permission)} disabled={permission === "OBSERVE"} onChange={() => togglePermission(permission)} className="mt-1 accent-[hsl(var(--primary))]" /><span><span className="block text-xs font-bold">{permission}</span><span className="mt-1 block text-[11px] leading-relaxed text-muted-foreground">{description}</span></span></label>)}</div></div><details className="mt-6 rounded-xl border border-border bg-muted/20 p-4"><summary className="cursor-pointer text-sm font-semibold">Advanced diagnostics <span className="ml-2 text-xs font-normal text-muted-foreground">technical scopes, endpoints, contract, and configuration</span></summary><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><p className="lee-label text-muted-foreground">Endpoint</p><p className="mt-1 break-all font-mono text-[11px] text-muted-foreground">{selected.diagnostics?.baseUrl ?? "Not configured"}{selected.diagnostics?.healthEndpoint ?? ""}</p></div><div><p className="lee-label text-muted-foreground">Contract</p><p className="mt-1 font-mono text-[11px] text-muted-foreground">{selected.diagnostics?.contractVersion ?? "Not reported"}</p></div><div><p className="lee-label text-muted-foreground">Granted scopes</p><p className="mt-1 break-all font-mono text-[11px] text-muted-foreground">{selected.diagnostics?.grantedScopes?.join(", ") || "Not applicable"}</p></div><div><p className="lee-label text-muted-foreground">Dependencies</p><p className="mt-1 text-[11px] text-muted-foreground">{selected.diagnostics?.dependencies?.length ?? 0} declared</p></div><pre className="max-h-48 overflow-auto rounded-lg bg-muted p-3 text-[10px] leading-relaxed text-muted-foreground sm:col-span-2">{JSON.stringify({ configuration: selected.diagnostics?.configuration ?? {} }, null, 2)}</pre></div></details><div className="mt-6 flex flex-wrap gap-2"><button onClick={() => void mutate(`/api/connections/${selected.id}/test`, { method: "POST", body: "{}" }, "Connection test completed.")} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3.5 py-2.5 text-xs font-semibold text-primary-foreground"><Check size={15} /> Test connection</button><button onClick={() => void startOAuth()} className="inline-flex items-center gap-2 rounded-xl border border-border px-3.5 py-2.5 text-xs font-semibold hover:bg-muted">{selected.method === "oauth" ? "Sign in" : "Reauthorize"}</button><button onClick={() => void mutate(`/api/connections/${selected.id}`, { method: "DELETE" }, "Connection disconnected safely.")} className="inline-flex items-center gap-2 rounded-xl border border-destructive/25 bg-transparent px-3.5 py-2.5 text-xs font-semibold text-destructive hover:bg-destructive/5"><Unplug size={15} /> Disconnect</button></div></> : <div className="flex min-h-72 items-center justify-center text-center"><div><ShieldCheck className="mx-auto mb-3 text-primary" size={24} /><p className="text-sm font-semibold">Choose a connection to inspect</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Permissions and diagnostics stay hidden until a system is selected.</p></div></div>}
        */}<ConnectionDetails selected={selected!} importing={importing} startOAuth={startOAuth} importFiles={importFiles} mutate={mutate} togglePermission={togglePermission} /></section>
    </div>}
