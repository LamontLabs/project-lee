...
test("all desktop packages include the relocatable PostgreSQL runtime", async () => {
  ...
  assert.match(installerTrust, /SystemCertificates\\\$storeName\\Certificates/);
  assert.doesNotMatch(installerTrust, /\.Default\.LocalMachine/);
  assert.match(installerTrust, /\.Flush\(\)/);
  assert.match(installerTrust, /registry-written-\$storeName/);
  assert.match(installerTrust, /registry-readback-\$storeName/);
  assert.match(installerTrust, /postwrite-\$storeName/);
