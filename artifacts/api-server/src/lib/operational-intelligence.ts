...
export async function generateOperationalContext() {
  const [initiatives, memory, world, objects] = await Promise.all([
    db.select().from(initiativeItem), operationalContext(), currentWorldState(), db.select().from(universalObject),
  ]);
  ...
  const atRiskItems = world.signals.filter((signal) => signal.signalType === "technical" || signal.currentValue?.alert).map((signal) => ({ id: signal.id, text: signal.signalName, value: signal.currentValue }));
  ...
  const [snapshot] = await db.insert(operationalContextSnapshot).values({ activePriority, changedItems, driftingItems, waitingItems, blockedItems: [], atRiskItems, canWaitItems, scoringContext: { operationalMemory: memory, worldStateSignals: world.signals.length, currentState: "live" } }).returning();
