...
export function normalizeAndroidSystemShare(input: unknown): PerceptionCapture | null {
  ...
  const sourceMetadata = boundedMetadata(value.sourceMetadata);
  const common = {
    captureId,
    purpose: `android_system_share_${captureType}`,
    capturedAt: capturedAt.toISOString(),
    authorizedScope: 'owner-private' as const,
    sourceMetadata: { ...sourceMetadata, sharedVia: 'android-system-share' },
    brainLinks: [],
  };
  ...
  if (!contentBase64 || !/^[A-Za-z0-9+/]*={0,2}$/.test(contentBase64) ||
    !Number.isFinite(declaredBytes) || declaredBytes < 1 || declaredBytes > MAX_SYSTEM_SHARE_BYTES ||
    decodedBytes < 1 || decodedBytes > MAX_SYSTEM_SHARE_BYTES) return null;
  ...
  } else if (captureType === 'document' && !supportedDocument(filename, mimeType)) {
    return null;
  }
  return {
    ...common,
    captureType,
    text: text || `Owner system share evidence: ${filename || 'shared content'}`,
    filename: filename || (captureType === 'image' || captureType === 'screenshot' ? 'lee-shared-image' : 'lee-shared-document'),
