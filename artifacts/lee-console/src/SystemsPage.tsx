...
type Connection = { id: string; displayName: string; targetType?: string; status: string; permissions?: string[]; lastHealthCheck?: string | null; lastError?: string | null };
...
export default function SystemsPage() {
  ...
  const layers = useMemo<Layer[]>(() => [
    ...
    { key: "connections", label: "Connections", description: "External provider access", status: optionalConnectionProblems.length ? "attention" : "healthy", detail: optionalConnectionProblems.length ? "Optional providers need attention; the canonical Brain remains separately reported." : connections.length ? "Configured providers have no current degraded status." : "No optional providers are configured yet.", items: connections.slice(0, 5).map((item) => `${item.displayName}: ${humanize(item.status)}`), href: "/connections" },
