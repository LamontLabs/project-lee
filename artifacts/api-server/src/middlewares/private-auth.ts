...
export function privateAuth(enabled = Boolean(process.env.LEE_OWNER_USERNAME && process.env.LEE_OWNER_PASSWORD)) {
  const middleware: RequestHandler = (req, res, next) => {
    ...
    if (req.path === "/contract" || req.path === "/api/contract" || req.path === "/system-contract" || req.path === "/api/system-contract" || req.path === "/contract.json" || req.path === "/api/contract.json" || req.path === "/recovery/status" || req.path === "/api/recovery/status" || (!desktopConfigured && !configured) || req.path.startsWith("/android/") && !req.path.startsWith("/android/pairing") || req.path.startsWith("/api/android/") && !req.path.startsWith("/api/android/pairing") || req.path === "/email/gmail/webhook" || req.path.endsWith("/health") || req.path.endsWith("/healthz") || req.path.endsWith("/auth/login") || req.path.endsWith("/auth/session") || req.path.endsWith("/auth/logout") || req.path.endsWith("/auth/enroll")) { next(); return; }
