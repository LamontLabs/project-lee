...
test("all desktop packages include the relocatable PostgreSQL runtime", async () => {
  ...
  assert.match(runtime, /command === process\.execPath/);
  ...
  assert.match(runtime, /LEE_SMOKE_MIGRATION_COMMAND/);
  assert.match(runtime, /this\.production && !smokeMigrationCommand/);
  ...
  assert.match(runtime, /this\.waitForContract\(\)[\s\S]*setTimeout\(\(\) => resolve\(null\), 10_000\)/);
  ...
  assert.match(main, /LEE_SMOKE_DISCOVERY_FILE/);
  ...
  assert.match(main, /smokeDiscoveryFile\)[\s\S]*await supervisor\.discoverLocalServices\(\)/);
  assert.match(main, /smokeExitRequested && !smokeUpdateFeedUrl && !smokeOwnerAuthFile/);
  assert.match(main, /smokePhase\("direct-boot"\)/);
  assert.match(main, /smokePhase\("supervisor-start-before"\)/);
  assert.match(main, /smokePhase\("supervisor-start-after"\)/);
  assert.match(main, /smokePhase\("headless-discovery-before"\)/);
  assert.match(main, /smokePhase\("headless-discovery-after"\)/);
  assert.match(main, /smokePhase\("headless-discovery-write-before"\)/);
  assert.match(main, /smokePhase\("headless-discovery-write-after"\)/);
  assert.match(main, /smokePhase\("direct-discovery-before"\)/);
  assert.match(main, /smokePhase\("direct-discovery-after"\)/);
  assert.match(main, /smokePhase\("direct-discovery-write-before"\)/);
  assert.match(main, /smokePhase\("direct-discovery-write-after"\)/);
  ...
  assert.match(main, /writeFileSync\(smokeDiscoveryFile, JSON\.stringify\(discovery, null, 2\), "utf8"\)/);
  ...
  assert.match(windowsSmoke, /LEE_SMOKE_HEADLESS"\] = "0"/);
