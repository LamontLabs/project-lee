...
export default function BriefTab() {
  ...
  useEffect(() => { void (async () => { const cached = await getBrief(); if (cached) setBrief(cached); if (api) try { const [live, health, confidence] = await Promise.all([api.brief(), api.health(), api.operationalConfidence()]); setBrief(live); setConnection(health); setOperationalConfidence(confidence); await saveBrief(live); setOffline(false); } catch { setConnection(null); setOffline(true); } })(); }, [api]);
  async function refresh() { if (!api) return; setRefreshing(true); try { const [live, health] = await Promise.all([api.brief(), api.health()]); setBrief(live); setConnection(health); await saveBrief(live); setOffline(false); } catch { setConnection(null); setOffline(true); } finally { setRefreshing(false); } }
  return (
