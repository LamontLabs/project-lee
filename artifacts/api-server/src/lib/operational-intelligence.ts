...
export async function generateOperationalContext() {
  const [initiatives, memory, world, objects, momentum, capacity] = await Promise.all([
  ...
  const [snapshot] = await db.insert(operationalContextSnapshot).values({ activePriority, changedItems, driftingItems: [...driftingItems, ...momentumDrift], waitingItems, blockedItems: [], atRiskItems, canWaitItems, scoringContext: { operationalMemory: memory, worldStateSignals: world.signals.length, momentumProjects: momentum.length, capacity: capacity.state, currentState: "live" } }).returning();
