...
import { contextPacket, db, eventLog, factLedger, interpretationLedger, universalObject, waitingLoop, trustScore } from "@workspace/db";
...
export async function buildContextPacket(query: string, mode: ConversationMode, budgetTokens = 3000) {
  const [objects, facts, interpretations, waiting, events, founder, trust] = await Promise.all([
