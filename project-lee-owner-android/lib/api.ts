...
export function createLeeApi(pairing: Pairing, manifest: ClientManifest = ownerManifest) {
  ...
  return {
    ...
    markNotificationDelivery: (notificationId: string, status: 'opened' | 'dismissed') => request<{ recorded: boolean }>(`/android/notifications/${encodeURIComponent(notificationId)}/delivery`, { method: 'POST', body: JSON.stringify({ status }) }),
