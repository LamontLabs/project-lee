...
test("publish workflow is limited to Windows and Linux release metadata", () => {
  ...
  assert.match(workflow, /HKCU:\\Software\\Microsoft\\SystemCertificates\\\$store\\Certificates/);
  assert.match(workflow, /Set-ItemProperty -Path \$certificateKey -Name Blob/);
  ...
  assert.doesNotMatch(workflow, /certutil\.exe|Import-Certificate|Import-PfxCertificate|StoreLocation\]::LocalMachine/);
