...
function headers(body: string, systemId: string, credential: string | undefined, correlationId: string) {
...
export async function registerUniversalSystem(input: UniversalSystemRegistration) {
  const [row] = await db.insert(internalCapabilityService).values({
    ...
    metrics: { capabilities: input.capabilities, universalSystemsApi: true },
    updatedAt: new Date(),
  }).onConflictDoUpdate({
...
export async function callUniversalSystem(systemId: string, path: string, payload: Record<string, unknown>, correlationId = randomUUID()) {
  ...
  if ((system.metrics as Record<string, unknown>)?.universalSystemsApi !== true) throw new Error("UNIVERSAL_SYSTEM_NOT_REGISTERED");
  if (!/^\/[a-zA-Z0-9/_-]*$/.test(path)) throw new Error("UNIVERSAL_SYSTEM_PATH_INVALID");
  const encoded = JSON.stringify({ contract_version: system.apiVersion, correlation_id: correlationId, payload });
  const credential = system.credentialEnvKey ? process.env[system.credentialEnvKey] : undefined;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 8000);
  try {
    const response = await fetch(`${system.baseUrl}${path}`, { method: "POST", headers: headers(encoded, system.serviceId, credential, correlationId), body: encoded, signal: controller.signal });
    if (!response.ok) throw new Error(`UNIVERSAL_SYSTEM_HTTP_${response.status}`);
