...
import {
  ...
  selfTestRun, sourceVault, universalObject, leeState,
} from "@workspace/db";
...
export async function generateManifest() {
  ...
  const [objects, facts, interpretations, people, assumptions, sources, events, engines, connectors, policies, provisions, indexes, backups, costs, routes, tests, states] = await Promise.all([
    ...
    db.select().from(selfTestRun).orderBy(desc(selfTestRun.startedAt)), db.select().from(leeState).limit(1),
  ]);
  ...
  const manifest: ManifestDocument = {
    ...
    health: { state: states[0]?.currentState ?? "Idle", overall: "nominal", criticalAlerts: 0, warnAlerts: 0, lastSelfTest: tests[0] ? { result: tests[0].overallResult, at: iso(tests[0].startedAt) } : null },
    dependencies: engines.map((engine) => ({ engine: engine.engineId, required: engine.dependencies, satisfied: engine.lifecycleState !== "UNAVAILABLE" })),
  };
