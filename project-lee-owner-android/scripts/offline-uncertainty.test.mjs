...
test("uncertainty refresh failure does not prevent a queued capture from saving", () => {
  ...
  const captureSync = Math.min(
    ...["createLeeApi(pairing).capture", "createLeeApi(pairing).perceptionCapture"]
      .map((needle) => {
        const index = context.indexOf(needle, addCaptureStart);
  ...
  const enqueueStart = context.indexOf("const enqueuePerception = async");
  assert.notEqual(enqueueStart, -1);
  assert.match(context.slice(enqueueStart, captureSync), /status: 'queued'/);
});